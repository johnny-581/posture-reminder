...
def draw_and_save_result(result: vision.PoseLandmarkerResult, output_image: mp, timestamp_ms: int):
    ...
    if result.pose_landmarks:
        for pose_landmarks in result.pose_landmarks:
            ...
            for landmark in pose_landmarks:
                # Only display for reasonably visible landmarks
                if landmark.visibility > 0.5:
                    # Convert normalized coordinates to pixel coordinates
                    pixel_x = int(landmark.x * image_width)
                    pixel_y = int(landmark.y * image_height)
                    
                    # Format the z-coordinate text (depth)
                    # The z-coordinate from MediaPipe is the depth, with the hip center as the origin.
                    # Smaller z-values mean the landmark is closer to the camera.
    ...
    cv2.putText(
        ...
        org=(10, 30),  # Position in the top-left corner
        fontFace=cv2.FONT_HERSHEY_SIMPLEX,
        fontScale=0.8,
        # fontScale=font_scale,
        color=(0, 255, 0),  # Green color for good visibility
        thickness=2,
